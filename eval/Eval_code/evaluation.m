function [allMets, metsBenchmark, metsMultiCam] = evaluation(allSequences, resDir, gtDataDir, benchmark)
...
metsBenchmark = [];
metsMultiCam = [];
% top_MOTP = zeros(1,0.5 * length(allSequences));
% hor_MOTP = zeros(1,0.5 * length(allSequences));
...
for ind = 1:length(allSequences)
    ...
    if ~multicam
        % MOTX parsing
        sequenceName = char(allSequences{ind}.name);
        sequenceFolder = [gtDataDir, filesep];
        fprintf('\t... %s\n',sequenceName);
%       assert(isfolder(sequenceFolder), 'Sequence folder %s missing\n', sequenceFolder);
        
        gtFilename = fullfile(gtDataDir,[sequenceName,'.txt']);
        gtdata = dlmread(gtFilename);
%         mask1 = gtdata(:, 1) > frames(1);
%         mask2 = gtdata(:, 1) < frames(2);
%         mask = mask1 & mask2;
%         gtdata = gtdata(mask(:,1), :);
        gtdata(:, 7:10) = -1;
        gtdata(gtdata(:,7)==0,:) = [];     % ignore 0-marked GT)
        gtdata(gtdata(:,1)<1,:) = [];      % ignore negative frames
        
        gtdata(gtdata(:,1)<allSequences{ind}.start,:) = [];  % filter the frames between start and end
        gtdata(gtdata(:,1)>allSequences{ind}.end,:) = []; 
    ...
    if ~multicam
        ...
        if exist(resFilename,'file')
            ...
            if s.bytes ~= 0
                ...
                resdata(resdata(:,1)<allSequences{ind}.start,:) = [];  % filter the frames between start and end
                resdata(resdata(:,1)>allSequences{ind}.end,:) = []; 
            else
                resdata = zeros(0,9);
            end
        else
    ...
    % Evaluate sequence
    [metsCLEAR, mInf, additionalInfo, MOTP] = CLEAR_MOT_HUN(gtMat{ind}, resMat{ind}, threshold, world);
    %%%%%%%%%% new added
    index = fix(ind/2);
    if mod(ind, 2) == 1
        top_MOTP(index + 1) = MOTP;
    else
        hor_MOTP(index) = MOTP;
    end
...
%     evalFile = fullfile(savepath, sprintf('eval_%s.txt',sequenceName));
...
% evalFile = fullfile(savepath, 'eval.txt');
...
%     evalFile = fullfile(savepath, 'eval_mc.txt');
